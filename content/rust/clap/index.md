...
features 列表: <https://docs.rs/clap/latest/clap/_features/index.html>

两种使用方式:

1.  derive macro: <https://docs.rs/clap/latest/clap/_derive/index.html>
    -   需要启用 derive/cargo feature，
2.  builder: <https://docs.rs/clap/latest/clap/builder/index.html>

builder 模式使用 macro 来快速定义 Comand/Arg：
...
crate_authors
: Allows you to pull the authors for the command from your Cargo.toml at compile time in the
    form:"author1 lastname &lt;author1@example.com&gt;:author2 lastname &lt;author2@example.com&gt;"
...
value_parser
: Select a `ValueParser` implementation from the intended type，默认是ValueParser::string；
...
fn main() {
    ...
    // command!() : requires `cargo` feature，从 Cargo.toml 中读取 name/version/authors/description信息
    let matches = command!()
    ...
    // You can check for the existence of subcommands, and if found use their matches just as you would the
    // top level cmd
...
1.  `value_parser!` for automatically selecting an implementation for a given type
    -   value_parser!() 参数是 `类型名称` , 如 std::path::PathBuf 或其他自定义类型等;
    -   clap::builder::ValueParserFactory 提供了 value_parser!() 可以使用的类型，如各种 Rust 原始类型；
2.  `ValueParser::new()` for additional `TypedValueParser` that can be used
    ...
    -   任何实现 TypedValueParser trait 的类型都可以用来创建 ValueParser;
...
clap::builder 提供了如下实现 `TypedValuePrarser` trait 的 struct 类型：
...
value_parser!() 为 Rust 基本类型和复杂类型创建 ValueParser，而 clap::builder 则为一些特殊类型预定义了实现
TypeValueParser trait 的类型 XXValueParser（如 NonEmptyStringValueParser) , 所以两者一般是结合使用的。

Arg::value_parser() 的方法签名：pub fn value_parser(self, parser: impl IntoResettable&lt;ValueParser&gt;) -&gt; Arg, 也就是任何实现 `IntoResettable<ValueParser>` 的类型，都可以作为 parser 参数：

-   value_parser!(T) for auto-selecting a value parser for a given type，Or range expressions like 0..=1 as a
    shorthand for RangedI64ValueParser
-   Fn(&amp;str) -&gt; Result&lt;T, E&gt;： 该类型闭包实现了 TypedValueParser trait.
...
// 例子: 使用函数(因为它实现了 TypeValueParser) 来作为 value_parser.
...
enum Command {
    Ping {
        /// Message to ping
        // 注：新 clap 版本使用  #[arg(value_parser = bytes_from_str)]
...
```rust
// 为数组 [P; C] 和 Vec<P> 转换为 ValueParser，一般用于
...
// 上面的 values 是任何可以迭代生成 PossibleValue 的迭代器，如 &[&str], Vec<&str>, [&str; N] 等；
...
TypedValueParser 是一个 trait，可以为自定义类型实现该 trait 来实现自定义解析，它们可以Into&lt;ValueParser&gt;，所以都可以作为 Arg::value_parser() 的参数：
...
let port: &PathBuf = m.get_one("output").expect("required");
...
-   derive 风格: Vec&lt;T&gt;
    ...
    -   默认为 T 自动添加 `.value_parser(value_parser!(T))`, 如果不符合预期则需要为自定义类型实现
        ValueParserFactory;
...
-   parse() 默认从 std::env::args_os 获取命令行参数, 使用 parse_from() 函数来传入其他命令行字符串来源;
...
Parser 可以使用 comand 和 arg attribue macro：
...
struct Cli {
    name: Option<String>, // 没有任何 attribute macro, 所以作为命令行位置参数
...
-   Arg.action() 的参数 [ArgAction](https://docs.rs/clap/latest/clap/enum.ArgAction.html) 默认为 `Set/SetTrue`, 对于 Vec 等类型需要明确设置为 `Append` ；
-   Arg field 的类型要求:
    -   Vec&lt;XX&gt; : 可以指定多次 flag, 各参数值被 Append 到 Vec 中;
    -   Option&lt;XX&gt;: 表示该 field 是可选的(默认是 `必选`);
...
对于 derive 风格的 subcommand，需要使用 enum 类型，每一个 struct variant 都是一个 subcommand，struct的 field
为 subcommand 的 args：
...
## <span class="section-num">4</span> 自定义 template {#自定义-template}
