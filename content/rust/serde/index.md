...
1.  `data structures` : 实现了 serialize 和 deserialize trait 的内置或自定义类型, 可以调用 serde
    crate 来实现;
...
data format 在各种单独的 serde_XX crate 中提供, 如 serde_json/serde_yaml 等, 它们实现了 data
structures 的 Serialize 和 Deserialize trait。
...
-   \#[serde(rename_all = "...")]
-   \#[serde(rename_all_fields = "...")] ，可以指定的值： "lowercase", "UPPERCASE", "PascalCase",
    "camelCase", "snake_case", "SCREAMING_SNAKE_CASE","kebab-case", "SCREAMING-KEBAB-CASE".
-   \#[serde(rename_all(serialize = "..."))]
-   \#[serde(rename_all(deserialize = "..."))]
-   \#[serde(rename_all(serialize = "...", deserialize = "..."))]

-   \#[serde(deny_unknown_fields)]: 在 deserialization 时如果遇到 unknown filed 则报错. (默认是忽略该
    field).
...
-   \#[serde(rename_all = "...")] ：可以指定的值："lowercase", "UPPERCASE", "PascalCase",
    "camelCase", "snake_case", "SCREAMING_SNAKE_CASE", "kebab-case", "SCREAMING-KEBAB-CASE".
...
-   \#[serde(with = "module")] # `Combination of serialize_with and deserialize_with`. Serde will use
    $module::serialize as the serialize_with function and $module::deserialize as the
...
核心是调用 serializer 提供的序列化方法，该 serializer 在后续调用 serde data format 如
serde_json/serde_yaml 时传入：
...
序列化 Rust 原始类型：serde 为 Rust 内置 29 种类型[都提供了 data structure 实现](https://serde.rs/data-model.html#types)
...
核心是调用 deserializer 的 deserialize_XX() 方法来反序列号，这些方法的参数是一个自定义的 Visitor
对象：
